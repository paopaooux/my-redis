...
struct AVLNode *rot_right(AVLNode *node) {
...
static AVLNode *avl_fix_left(AVLNode *root) {
    if (avl_depth(root->left->left) < avl_depth(root->left->right)) { // LR
...
static AVLNode *avl_fix_right(AVLNode *root) {
    if (avl_depth(root->right->right) < avl_depth(root->right->left)) { // RL
        root->right = rot_right(root->right);
    }
    return rot_left(root);
}

// 修复不平衡节点并维护不变量，直到到达根节点
